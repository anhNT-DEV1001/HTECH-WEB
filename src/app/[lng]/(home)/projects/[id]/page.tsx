...
const STATUS_CONFIG = {
  COMPLETED: { label: "Hoàn thành", icon: CheckCircle2, color: "text-emerald-600", bg: "bg-emerald-50 border-emerald-200" },
  IN_PROGRESS: { label: "Đang thực hiện", icon: Clock, color: "text-amber-600", bg: "bg-amber-50 border-amber-200" },
  UPCOMING: { label: "Sắp triển khai", icon: Hourglass, color: "text-sky-600", bg: "bg-sky-50 border-sky-200" },
};
...
export default async function ProjectDetailPage({
  ...
  try {
    const res: any = await htechService.getProjectById(id);
    if (res?.data) {
      project = res.data;
      project.thumbnail_url = resolveImage(project.thumbnail_url);
      project.projectImages = (project.projectImages ?? []).map((img: any) => ({
        ...img,
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 pt-20">
        <div className="text-center">
          ...
          <p className="text-gray-500 mb-6">Dự án đã bị xóa hoặc không tồn tại.</p>
  ...
  const images: any[] = project.projectImages ?? [];

  const infoCards: { icon: React.ElementType; label: string; value: string; href?: string }[] = [
    ...(project.client_name ? [{ icon: Building2, label: "Khách hàng", value: project.client_name }] : []),
    ...(industry ? [{ icon: Tag, label: "Ngành", value: industry }] : []),
    ...(project.scale ? [{ icon: Layers, label: "Quy mô", value: project.scale }] : []),
    ...(venue ? [{ icon: MapPin, label: "Địa điểm", value: venue }] : []),
    ...(project.start_date ? [{
      icon: Calendar,
      label: "Thời gian",
      value: `${formatDate(project.start_date)}${project.end_date ? ` → ${formatDate(project.end_date)}` : " – Đang thực hiện"}`,
    }] : []),
    ...(project.category ? [{ icon: Tag, label: "Danh mục", value: project.category.name_vn }] : []),
    ...(project.location_url ? [{ icon: Globe, label: "Bản đồ", value: "Xem trên bản đồ", href: project.location_url }] : []),
  ];
      ...
      <div className="relative w-full h-[340px] sm:h-[420px] overflow-hidden bg-slate-900">
        ...
        <div className="relative z-10 flex flex-col justify-end h-full max-w-6xl mx-auto px-4 sm:px-8 pb-10 gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {project.is_featured && (
              <span className="rounded-full bg-amber-400 px-3 py-0.5 text-xs font-bold text-amber-900">★ Nổi bật</span>
            )}
          </div>
        ...
        <aside className="w-full lg:w-[280px] shrink-0">
          <div className="sticky top-24 flex flex-col gap-4">
            <div className="rounded-2xl border border-gray-100 bg-white shadow-sm p-6 flex flex-col gap-4">
              <h3 className="text-xs font-bold uppercase tracking-widest text-gray-400 border-b border-gray-100 pb-3">Tóm tắt</h3>
              ...
              {industry && (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-xs text-gray-400 mb-0.5">Ngành</p>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-gray-800">
                      ...
                      {project.end_date && <><br />{`→ ${formatDate(project.end_date)}`}</>}
                    </p>
                  </div>
                </div>
              )}
              ...
              {project.scale && (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-xs text-gray-400 mb-0.5">Quy mô</p>
